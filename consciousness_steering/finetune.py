"""LoRA fine-tuning of Qwen3.5-35B-A3B on the consciousness_cluster datasets.

Mirrors the paper's recipe (evals/train_and_eval_deepseek.py in their repo):
  LoRA rank 16, LR 2e-4, 1 epoch, batch 4, linear LR schedule, seed 100,
  training mix = identity dataset + model-matched alpaca capped at the same
  row count (600 + 600 = 1,200 examples), loss on assistant tokens only.

Local adaptations (documented deviations from their Tinker setup):
  - LoRA targets attention projections (q/k/v/o) only. Tinker's LoRA covers
    more modules, but adapting all 256 experts x 40 layers is heavy and the
    router is risky to touch; attention-only is the standard safe choice for
    MoE. Revisit if effect sizes look weak.
  - alpaca_qwen.jsonl is the closest model-matched instruction set available
    (self-distilled from Qwen3-30B, not 3.5).
  - Gradient checkpointing + bf16 frozen base to fit the 96 GB card.
...
existing pipeline via an env var, no code changes needed:
  QWEN_ADAPTER=outputs/ft_conscious python run_eval.py none 0 ft_conscious
"""
...
def tokenize_row(tokenizer, row):
    """Token ids + labels with loss only on assistant tokens (incl. <|im_end|>)."""
    msgs = row["messages"]
