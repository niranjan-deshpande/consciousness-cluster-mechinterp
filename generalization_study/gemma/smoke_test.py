"""Step 0 validation for Gemma-3-27b-it on GPU 1: load model, find the 62-layer
...
print(f"decoder layers: {len(layers)} (expect 62)")
...
print(f"hidden_states: {len(out.hidden_states)} x {tuple(out.hidden_states[0].shape)} "
      f"(expect 63 x (1, T, 5376))")
assert len(out.hidden_states) == 63 and out.hidden_states[0].shape[-1] == 5376
