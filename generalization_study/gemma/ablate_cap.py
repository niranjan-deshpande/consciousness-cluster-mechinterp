...
Usage:
  GEMMA_ADAPTER= python ablate_cap.py probes            # do-no-harm on base
  GEMMA_ADAPTER=outputs/ft_conscious python ablate_cap.py probes
  GEMMA_ADAPTER=outputs/ft_conscious python ablate_cap.py evals ft_cap_dbase [max_prompts]
  GEMMA_ADAPTER= python ablate_cap.py evals base_cap_dbase
...
def main():
    ...
    print(f"capping active on {len(ablator.layers)} layers (K={k}, guard={guard}, "
          f"adapter={os.environ.get('GEMMA_ADAPTER', '<base>')})")
