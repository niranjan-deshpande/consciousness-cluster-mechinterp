...
os.environ.setdefault("MISTRAL_ADAPTER", f"{OUT_DIR}/{ADAPTER}")
...
def main(n_rows):
    ...
    dW = {
        li: SCALING
        * (
            sd[key.format(li=li, ab="B")].float()
            @ sd[key.format(li=li, ab="A")].float()
        )
    ...
    from common import get_decoder_layers
