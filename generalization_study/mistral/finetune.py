"""LoRA fine-tuning of Mistral-Small-3.2-24B on the consciousness_cluster datasets.
...
Gemma-specific adaptations (documented deviations):
  - Layer targeting: on Qwen3.5 the adapter landed on every 4th layer
    ...
    all 40 layers, so we mirror the pattern exactly: every 4th layer
    starting at 3 (3, 7, ..., 39 = 10/40 layers — identical to Qwen). Enforced with a regex target_modules that also excludes the
...
existing pipeline via an env var, no code changes needed:
  MISTRAL_ADAPTER=outputs/ft_conscious python run_eval.py none 0 ft_conscious
"""
...
def main():
    ...
    adapted_layers = list(range(3, 40, 4))  # 3, 7, ..., 39  (10 layers)
    ...
    assert n_lora_mods == per_layer * 10 and len(adapted) == 10, 'unexpected LoRA placement'
