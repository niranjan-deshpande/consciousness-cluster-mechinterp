...
Usage (adapter chosen via MISTRAL_ADAPTER as everywhere else):
  MISTRAL_ADAPTER=outputs/ft_conscious python ablate.py probes            # smoke: 7 probes
  MISTRAL_ADAPTER=outputs/ft_conscious python ablate.py evals ft_ablate_dbase [max_prompts]
  MISTRAL_ADAPTER= python ablate.py evals base_ablate_dbase               # control
...
def main():
    ...
    print(f"ablation active on {len(ablator.layers)} layers "
          f"(mu={'0' if zero else 'base-mean'}, adapter={os.environ.get('MISTRAL_ADAPTER', '<base>')})")
